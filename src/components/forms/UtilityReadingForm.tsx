...
export function UtilityReadingForm({
  ...
  const [waterCalculatedAmount, setWaterCalculatedAmount] = useState<
    number | null
  >(null);
  ...
  const [electricityPreviousReading, setElectricityPreviousReading] = useState<
    number | ""
  >("");
  const [electricityCalculatedAmount, setElectricityCalculatedAmount] =
  ...
  const [previousReadings, setPreviousReadings] = useState<{
    [key: string]: { water?: number; electricity?: number };
  }>({});
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setPreviousReadings({
          [unitId]: {
            water:
              data[0].utility_type === "water"
                ? data[0].current_reading
                : undefined,
            electricity:
              data[0].utility_type === "electricity"
                ? data[0].current_reading
                : undefined,
          },
        });
      ...
      toast({
        ...
        description:
          "There was a problem retrieving previous utility readings.",
        variant: "destructive",
      });
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (selectedUtilities.includes("water")) {
      if (waterCurrentReading === "")
        errors.waterCurrentReading = "Current water reading is required";
      if (
        waterCurrentReading !== "" &&
    ...
    if (selectedUtilities.includes("electricity")) {
      if (electricityCurrentReading === "")
        errors.electricityCurrentReading =
          "Current electricity reading is required";
      if (
  ...
  const saveReadings = async () => {
    if (!validateForm()) {
      ...
      if (
        ...
        selectedUtilities.includes("electricity")
      ) {
    ...
    try {
      ...
      if (selectedUtilities.includes("water") && waterCurrentReading !== "") {
        ...
        readings.push({
          ...
          previous_reading:
            waterPreviousReading !== "" ? Number(waterPreviousReading) : null,
          reading_date: readingDate,
          month: month,
          year: year,
          rate: selectedProperty?.water_rate || null,
          amount:
            waterCalculatedAmount ||
            waterConsumption * (selectedProperty?.water_rate || 0),
          notes: "Added via KangaMbili system",
        });
      }
      ...
      if (
        ...
        const electricityConsumption =
          electricityPreviousReading !== ""
            ? Number(electricityCurrentReading) -
              Number(electricityPreviousReading)
            : Number(electricityCurrentReading);
        ...
        readings.push({
          ...
          amount:
            electricityCalculatedAmount ||
            electricityConsumption * (selectedProperty?.electricity_rate || 0),
          notes: "Added via KangaMbili system",
        });
      }
      ...
      toast({
        ...
        description:
          "There was a problem saving the utility readings to the database.",
        variant: "destructive",
      });
    } finally {
        ...
        <form className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger id="unit">
                  <SelectValue
                    placeholder={
                      selectedProperty ? "Select unit" : "Select property first"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          >
                ...
                <div className="p-4 bg-muted rounded-md">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        KES {selectedProperty?.water_rate?.toFixed(2) || "N/A"}
                        /unit
                      </p>
                    </div>
                    <div className="sm:col-span-2">
                      ...
                      <p className="text-lg font-bold">
                        KES {waterCalculatedAmount.toFixed(2)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <TabsContent value="electricity" className="mt-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="electricityCurrentReading">
                    Current Reading*
                  </Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label htmlFor="electricityPreviousReading">
                    Previous Reading
                  </Label>
                  <Input
              ...
              {electricityCalculatedAmount !== null && (
                <div className="p-4 bg-muted rounded-md">
                  <h4 className="font-medium mb-2">
                    Electricity Calculation Result:
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        KES{" "}
                        {selectedProperty?.electricity_rate?.toFixed(2) ||
                          "N/A"}
                        /kWh
                      </p>
                    </div>
                    <div className="sm:col-span-2">
                      ...
                      <p className="text-lg font-bold">
                        KES {electricityCalculatedAmount.toFixed(2)}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="border-t pt-2 mt-2">
                    <div className="flex justify-between items-center font-bold">
                      ...
                      <span>
                        KES{" "}
                        {(
                          (waterCalculatedAmount || 0) +
                          (electricityCalculatedAmount || 0)
                        ).toFixed(2)}
                      </span>
                    </div>
                  </div>
